...
router.get('/cases/:caseId/intelligence-assessments',authenticate,readers,async(req,res)=>{try{return res.json({success:true,data:await getCaseIntelligenceHistory(String(req.params.caseId),req.user!,{limit:parseLimit(req.query.limit),cursor:parseCursor(req.query.cursor)})});}catch(error){const invalid=queryError(res,error);return invalid??fail(res,error);}});
