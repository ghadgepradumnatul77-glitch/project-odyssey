...
router.post('/orps/:orpId/decisions', authenticate, requireRole(SystemRole.OFFICER), async (req, res) => {
  try {
...
router.get('/orps/:orpId/decisions', authenticate, async (req, res) => {
  try {
    const orpId = requiredText(req.params.orpId);
...
router.get('/cases/:caseId/decisions', authenticate, async (req, res) => {
  try {
